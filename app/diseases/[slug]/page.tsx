                ...
                {disease.ccrhEvidence.keyFindings?.length > 0 && (
                  <ul style={{ marginTop: 12, paddingLeft: 0, listStyle: 'none' }}>
                    {disease.ccrhEvidence.keyFindings.map((f: string, i: number) => (
                      <li key={i} style={{ display: 'flex', gap: 8, marginBottom: 6, fontSize: 13, color: 'var(--ink2)' }}>
          ...
          {disease.sources?.length > 0 && (
            <section style={{ marginBottom: 40 }}>
              ...
              {disease.sources.map((s: any, i: number) => (
                <div key={i} style={{ display: 'flex', gap: 8, fontSize: 12, color: 'var(--ink4)', marginBottom: 6, alignItems: 'flex-start' }}>
                  <span style={{ flexShrink: 0 }}>[{i + 1}]</span>
                  <span>
                    {s.url ? (
                      <a href={s.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--gold-dk)', textDecoration: 'underline' }}>{s.name}</a>
                    ) : s.name}
                    {s.year && ` (${s.year})`}
                  </span>
                </div>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px', marginBottom: 16 }}>
              ...
              {related.map((r: any) => (
                <Link key={r.slug?.current} href={`/diseases/${r.slug?.current}`} style={{ display: 'block', padding: '10px 0', borderBottom: '1px solid var(--border)', textDecoration: 'none', fontSize: 13, color: 'var(--ink2)', fontWeight: 500 }}>
                  {r.title}
                  {r.hindiName && <span style={{ display: 'block', fontSize: 11, color: 'var(--ink4)', fontWeight: 300, fontStyle: 'italic', marginTop: 2 }}>{r.hindiName}</span>}
                </Link>
              ))}
            </div>
          )}
